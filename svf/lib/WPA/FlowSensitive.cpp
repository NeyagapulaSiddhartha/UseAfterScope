...
void FlowSensitive::solveConstraints()
{
    ...
    do
    {
        ...
        initWorklist();
        std::cout<<"FlowSensitive::initWorklist\n";
        solveWorklist();
    }
...
void FlowSensitive::analyze()
{
    ...
    else
    {
        ...
        {
            std::cout<<" FlowSensitive::analyze\n";
            initialize();
            std::cout<<" FlowSensitive::analyze\n";
            solveConstraints();
            std::cout<<" FlowSensitive::constraints\n";
            finalize();
            std::cout<<" FlowSensitive::finalise\n";
        }
        else
        {
...
{
    std::cout<<"FlowSensitive::finalize\n";
    if(Options::DumpVFG())
...
{
    //std::cout<<" inside the propFRpm \n";
    if(isa<ThreadMHPIndSVFGEdge>(edge)){

        //std::cout<<" the edge is threadMHPIndirectVFGEdge\n";
    }
    else{
       // std::cout<<" the edge is not threadMHPIndirectVFGEdge\n";
    }
...
bool FlowSensitive::processAddr(const AddrSVFGNode* addr)
{
    ...
    // std::cout<<" process Addr :: "<<addr->getValue()->toString()<<" "<< addr->getPAGDstNodeID()<<" ------>"<<srcID<<" \n";
...
bool FlowSensitive::processLoad(const LoadSVFGNode* load)
{
    ...
    {
         std::cout<< " load src and dest "<<load->getValue()->toString()<<"  "<<load->getPAGSrcNodeID()<<" ----->" <<load->getPAGDstNodeID()<<"   \n";
