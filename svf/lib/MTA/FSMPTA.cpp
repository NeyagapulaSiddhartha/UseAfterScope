...
{
    SVFUtil::outs() << "stage 1 FSMPTA adding edge\n";
    MemSSA* mssa = svfg->getMSSA();
    SVFUtil::outs() << "stage 2 FSMPTA adding edge\n";
    ...
    {
        SVFUtil::outs() << "stage 11 FSMPTA adding edge\n";
        SVFUtil::outs() << "stage 12 FSMPTA adding edge\n";
...
void MTASVFGBuilder::collectLoadStoreSVFGNodes()
{
    ...
    for (SVFG::const_iterator it = svfg->begin(), eit = svfg->end(); it != eit; ++it)
    {
        ...
        if (SVFUtil::isa<LoadSVFGNode>(snode))
        {
            ...
            if (node->getInst())
            {
                // std::cout<<" printting inside thecollectLoadSVFGNodes " <<node->getValue()->toString()<<"\n";
        ...
        if (SVFUtil::isa<StoreSVFGNode>(snode))
        {
            ...
            if (node->getInst())
            {
                // std::cout<<" printting inside thecollectStoreSVFGNodes " <<node->getValue()->toString()<<"\n";
...
void MTASVFGBuilder::performAddingMHPEdges()
{
    ...
    std::cout<< "Added " << MTASVFGBuilder::numOfNewSVFGEdges << " SVFG edges\n";
}

SVFGEdge*  MTASVFGBuilder::addTDEdges(NodeID srcId, NodeID dstId, PointsTo& pts)
{
    // std::cout<<" adding the thread edges \n";
    ...
    {
        std::cout<<"the edge is already present \n";
    ...
    {
        std::cout<<"the edge is not present \n";
        ...
        {
            // std::cout
            // std::cout<<"the source node is "<<srcNode->getValue()->toString()<<"\n";
            //  std::cout<<"the destination node is "<<dstNode->getValue()->toString()<<"\n";

            // std::cout<<"the edge is *****************   "<<indirectEdge->getEdgeKind()<<"    ************************ added \n";
            return indirectEdge;
        }
        else
        {
...
void FSMPTA::initialize(SVFModule* module)
{
    ...
    MTASVFGBuilder* mtaSVFGBuilder = new MTASVFGBuilder(mhp,lockana);
...
//    std::cout<<"stage 3 FSMPTA adding edge\n";
//     // svfg=mtaSVFGBuilder.getSVFG();
//     setGraph(svfg);
//     //AndersenWaveDiff::releaseAndersenWaveDiff();
...
void FSMPTA::analyze(){
    ...
    std::cout<<" user defined FSMPTA::analyze\n";
    solveConstraints();
    std::cout<<" FSMPTA::constraints\n";
    finalize();
    std::cout<<" FSMPTA::finalise\n";
}
