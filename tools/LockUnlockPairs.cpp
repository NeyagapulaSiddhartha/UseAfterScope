...
void LockUnlockPairs::visit(Module &M, FunctionAnalysisManager &FAM) {
	...
	for (unsigned j = 0; j < calls.getCallsSizeAt(i) - 1; j++) {
		...
		if (checkMutexAlias(inst1, inst2, FAM)) {
...
bool LockUnlockPairs::checkMutexAlias(CallInst *call1, CallInst *call2, FunctionAnalysisManager &FAM) {
    Value *mutex1;
    Value *mutex2;
	...
	auto &AA = FAM.getResult<AAManager>(*call1->getFunction());
 llvm::AliasResult   res = AA.alias(mutex1, mutex2);
...
void LockUnlockPairs::printDebugInfo() const {
	...
	if (metaNode1) {
	    if (auto *Loc = llvm::dyn_cast<llvm::DILocation>(metaNode1)) {
            StringRef File = Loc->getFilename();
	...
	if (metaNode2) {
	    if (auto *Loc = llvm::dyn_cast<llvm::DILocation>(metaNode2)) {
            StringRef File = Loc->getFilename();
            unsigned Line = Loc->getLine();
            llvm::errs() << File << '\t' << Line << '\n';
        }
