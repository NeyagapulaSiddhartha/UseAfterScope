...
#include "/home/cs23mtech12010/CCM/SVF-SVF-3.0/svf/include/MTA/LockAnalysis.h"
...
void WPAPass::runPointerAnalysis(SVFIR* pag, u32_t kind)
{
    ...
    TCT * tct = new TCT(_pta);
    MHP * mhp = new MHP(tct);
    mhp->analyze();

    LockAnalysis *lsa = new LockAnalysis(tct);
    // MTASVFGBuilder *svf=new MTASVFGBuilder(mhp,lsa);
    // _svfg=svf->getSVFG();
                            // _svfg->build(_pta ,VFG::VFGK::FULLSVFG);
    // pag = svf->build();
    FSMPTA * fsmpta = FSMPTA::createFSMPTA(pag->getModule(), mhp, lsa);
...
void WPAPass::PrintAliasPairs(PointerAnalysis* pta)
{
    ...
    for (SVFIR::iterator lit = pag->begin(), elit = pag->end(); lit != elit; ++lit)
    {
        ...
        for (SVFIR::iterator rit = lit, erit = pag->end(); rit != erit; ++rit)
        {
            ...
            if(result == AliasResult::MayAlias && node1->getValue() && node2->getValue())
           { 
